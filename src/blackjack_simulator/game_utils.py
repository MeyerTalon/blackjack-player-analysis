...
def _format_card(card: Any) -> str:
    """Best-effort string for a card object with attributes rank/suit/val."""
...
def build_players(n_players: int, player_types: List[str], model: str, temperature: float):
    """
    player_types: list like ["basic","aggressive","random",...]
    - "random" -> RandomPlayer()
    - anything else -> LLMBlackjackPlayer(persona=<type>, model, temperature)
    If len(player_types) == 1, the same type is replicated across seats.
    """
...
def build_base_bets(n_players: int, base_bet_value: float = None, base_bets_list: List[float] = None) -> List[float]:
    """
    Provide either a single --base-bet value to replicate across players,
    or a comma list via --base-bets like '10,10,25,10'.
    """
